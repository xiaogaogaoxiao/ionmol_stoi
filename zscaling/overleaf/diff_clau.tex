...
%DIF DEL ../zscaling.tex         Thu May 21 13:36:36 2020
%DIF ADD zscaling_overleaf.tex   Tue May 19 16:03:56 2020
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\date{\DIFaddbegin \today\DIFaddend }% It is always \today, today,
...
We investigate \DIFdelbegin \DIFdel{the scaling }\DIFdelend %DIF > the scaling 
\DIFaddbegin \DIFadd{scaling rules }\DIFaddend of the ionization cross section with 
...
Combining these two features, we define a scaling law  \DIFdelbegin \DIFdel{independent of 
the ion charge and the complexity of the molecular target }\DIFdelend \DIFaddbegin \DIFadd{for any ion and molecular target %DIF > independent of  the ion charge and the complexity of the molecular target
}\DIFaddend , which is 
valid in the intermediate to high energy range\DIFdelbegin \DIFdel{. Then}\DIFdelend \DIFaddbegin \DIFadd{, i.e., 0.2-5 MeV/amu for oxygen impact. %DIF > Then, 
Thus}\DIFaddend , the forty 
...
This contribution constitutes a follow-up \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend our previous work\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
...
considering }\DIFdelend %DIF > depends linearly with ion charge $Z$, considering 
...
Tribedi_O_water} with the $Z^\alpha$-scaling rule. \DIFaddbegin \DIFadd{For targets with none or little experimental data, we included
...
conversion.
}\DIFaddend As can be noted, 
most of the data in Fig.~\ref{fig:zreduced} confirm the present 
scaling, even for O$^{+8}$ in water~\cite{Tribedi_O_water}. 
Only two data sets are off our predictions: the ionization \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{cross sections }\DIFaddend of 
uracil by swift C, O, and F ions from 
Refs\DIFdelbegin \DIFdel{.~\mbox{%DIFAUXCMD
\cite{agnihotri2012,agnihotri2013}}\hspace{0pt}%DIFAUXCMD
are too low 
compared with our CDW-SSM results, and the data }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{agnihotri2012,agnihotri2013}}\hspace{0pt}%DIFAUXCMD
%DIF >  are too low  compared with our CDW-SSM results
; and the values 
}\DIFaddend for Li$^{+3}$ in water from Ref.~\cite{Luna_Li_water} \DIFdelbegin \DIFdel{spread out from the present theoretical 
curves }\DIFdelend %DIF > spread out from the present theoretical curves
for $E<600$~keV/amu. 
\DIFdelbegin \DIFdel{For targets with none or little experimental data, we included 
electron impact ionization results~\mbox{%DIFAUXCMD
\cite{rahman2016,bug2017,wolf2019,
fuss2009} }\hspace{0pt}%DIFAUXCMD
at high velocity with the corresponding equivelocity 
conversion}\DIFdelend \DIFaddbegin \DIFadd{In the case of uracil, recent CTMC calculations by Sarkadi~\mbox{%DIFAUXCMD
\cite{sarkadi2016} }\hspace{0pt}%DIFAUXCMD
are also above the experimental values by Tribedi group~\mbox{%DIFAUXCMD
\cite{agnihotri2012,agnihotri2013}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{sections, }\DIFaddend $\sigma_A$\DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{, of atomic targets }\DIFaddend H, C, N, \DIFdelbegin \DIFdel{O targets }\DIFdelend \DIFaddbegin \DIFadd{and O  }\DIFaddend scale with the number of
active electrons per atom $\nu_A$, as \mbox{$\sigma_e=\sigma_A/\nu_A$,} 
where $\nu_A$ is $1$ for H and $4$ for C, N, \DIFdelbegin \DIFdel{and }\DIFdelend O, i.e.,
\begin{equation}
...
\end{equation}
By \DIFdelbegin \DIFdel{considering }\DIFdelend %DIF > considering
\DIFaddbegin \DIFadd{means of }\DIFaddend the SSM, we define the number of active electrons per
molecule as $n_e=\sum_A n_A \nu_A$. The $n_e$ values for the 
molecular targets considered \DIFdelbegin \DIFdel{throughout }\DIFdelend \DIFaddbegin \DIFadd{throughtout }\DIFaddend this work are displayed in 
...
By incorporating the $Z^\alpha$ reduction and the \DIFdelbegin \DIFdel{molecular }\DIFdelend \DIFaddbegin \DIFadd{scaling with the %DIF > molecular 
}\DIFaddend number  of active \DIFdelbegin \DIFdel{electron scaling}\DIFdelend \DIFaddbegin \DIFadd{electrons%DIF >  scaling
}\DIFaddend , we introduce the scaled and reduced
ionization cross section of molecules $\tilde{\sigma}$, \DIFaddbegin \DIFadd{which is 
}\DIFaddend expressed as a function of $E/Z^{2-\alpha}$, and \DIFaddbegin \DIFadd{it is }\DIFaddend given by
\begin{equation}
...
Table~\ref{nn} \DIFdelbegin \DIFdel{, }\DIFdelend and the parameter is $\alpha=1.2$.
Fig.~\ref{fig:zalpha} shows the \DIFdelbegin \DIFdel{reduced-scaled }\DIFdelend %DIF > reduced-scaled 
theoretical and 
experimental \DIFdelbegin \DIFdel{data from Fig.~\ref{fig:zreduced} obtained using 
}\DIFdelend \DIFaddbegin \DIFadd{values of $ \tilde{\sigma}$ (given by }\DIFaddend Eq.~(\ref{eq:u-scaling})\DIFaddbegin \DIFadd{) %DIF > data 
for all the systems displayed in Fig.~\ref{fig:zreduced}%DIF >  obtained using Eq.~(\ref{eq:u-scaling})
}\DIFaddend . As can be noted, the scaling works very well
...
which is \DIFdelbegin \DIFdel{shown with }\DIFdelend \DIFaddbegin \DIFadd{coverd by %DIF > shown with 
}\DIFaddend the gray area \DIFaddbegin \DIFadd{in Fig.~\ref{fig:zalpha}}\DIFaddend .
It is worth noting that we did not include in \DIFdelbegin \DIFdel{Fig.~\ref{fig:zreduced}
}\DIFdelend \DIFaddbegin \DIFadd{this figure %DIF > Fig.~\ref{fig:zalpha}
}\DIFaddend the data for uracil from Refs.~\cite{agnihotri2012,agnihotri2013}, 
and \DIFaddbegin \DIFadd{for }\DIFaddend Li$^{+3}$ on water~\cite{Luna_Li_water}. The discussion 
...
We consider \DIFdelbegin \DIFdel{our scaling law }\DIFdelend \DIFaddbegin \DIFadd{this scaling }\DIFaddend robust enough to be valid for different 
\DIFdelbegin \DIFdel{ion-molecule }\DIFdelend \DIFaddbegin \DIFadd{ion--molecule }\DIFaddend combinations. We tested the generality of our model 
by including in Fig.~\DIFdelbegin \DIFdel{\ref{fig:zreduced} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:zalpha} }\DIFaddend several data sets of 
...
in N$_2$, O$_2$, CH$_4$, CO and CO$_2$, and the \DIFdelbegin \DIFdel{most recent data }\DIFdelend \DIFaddbegin \DIFadd{recent values by 
Luna~}\textit{\DIFadd{et al.}} \DIFadd{\mbox{%DIFAUXCMD
\cite{Luna2019} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend for H$^{+}$ in CH$_4$\DIFdelbegin \DIFdel{by Luna~}\textit{\DIFdel{et al.}} %DIFAUXCMD
\DIFdel{\mbox{%DIFAUXCMD
\cite{Luna2019}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend . 

The good agreement shown in Fig.~\ref{fig:zalpha} \DIFdelbegin \DIFdel{summarizes }\DIFdelend \DIFaddbegin \DIFadd{summaries }\DIFaddend the main 
result of this work\DIFaddbegin \DIFadd{, }\DIFaddend and holds the validity of the present scaling \DIFdelbegin \DIFdel{with the ion charge and the number of active electrons in the target.
}\DIFdelend \DIFaddbegin \DIFadd{for different ions and targets.
%DIF > with the ion charge and the number of active electrons in the target. 
}\DIFaddend Although the theoretical CDW-SSM results are valid for energies above 
the maximum of the cross sections, \DIFdelbegin \DIFdel{it is worth noting from 
Fig.~\ref{fig:zalpha} that }\DIFdelend %DIF > it is worth noting from Fig.~\ref{fig:zalpha} that 
the scaling of the experimental data extends 
even to lower impact energies\DIFaddbegin \DIFadd{, as can be noted in Fig.~\ref{fig:zalpha}}\DIFaddend . 
...
scaling considers the molecular \DIFdelbegin \DIFdel{characteristics }\DIFdelend \DIFaddbegin \DIFadd{characteristic }\DIFaddend of the target by taking 
into account the number of active electrons per molecule, $n_e$. 
The last scaling law combines the $Z^{\alpha}$-reduction with the
$n_e$-scaling 
of \DIFdelbegin \DIFdel{the }\DIFdelend cross section, \DIFdelbegin \DIFdel{becoming }\DIFdelend \DIFaddbegin \DIFadd{and it becomes }\DIFaddend independent of the ion charge and 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\DIFdelbegin \DIFdel{Acknowledgments}\DIFdelend \DIFaddbegin \DIFadd{Acknowlegments}\DIFaddend }
...
\bibitem{sarkadi2016} L. Sarkadi, J. Phys. B: At. Mol. Opt. Phys. \textbf{49} 185203 (2016)\DIFaddbegin \DIFadd{.
}\DIFaddend 
